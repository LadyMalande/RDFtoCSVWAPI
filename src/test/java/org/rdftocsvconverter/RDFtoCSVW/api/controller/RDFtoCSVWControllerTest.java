...
class RDFtoCSVWControllerTest {
    ...
    void testGetCSVW() throws Exception {
        CompletableFuture<byte[]> mockResponse = CompletableFuture.completedFuture("Mock CSVW content".getBytes());
        ...
        mockMvc.perform(MockMvcRequestBuilders.multipart("/rdftocsvw").file(file).param("fileURL", "https://raw.githubusercontent.com/LadyMalande/RDFtoCSVNotes/refs/heads/main/examples/MissingDataExample.ttl").param("choice", String.valueOf(ParsingChoice.RDF4J)).param("tables", String.valueOf(TableChoice.ONE)).param("firstNormalForm", "true")).andExpect(status().isOk()).andExpect(content().bytes(mockResponse.get()));
    }
